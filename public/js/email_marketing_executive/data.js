...
$(function (){
    ...
    DATA_TABLE = $('#table-data').DataTable({
        ...
        "serverSide": true,
        "ajax": {
            "url": URL + '/email-marketing-executive/data/get-eme-data',
            data: {
                filters: function (){
                    let obj = {
                    };
        ...
        "columns": [
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    ...
                    //html += '<a href="'+URL+'/email-marketing-executive/data/take-lead-data/'+ $('meta[name="ca-agent-id"]').attr('content') + '/' + btoa(row.id) +'" class="btn btn-outline-success btn-rounded btn-sm" title="Take Lead"><i class="feather icon-share mr-0"></i></a>';
                    return html;
                }
            },
            {
    ...
    $('#form-edit-data-submit').on('click', function (e) {
        ...
        $.ajax({
            type: 'post',
            url: URL + '/email-marketing-executive/data/update/' + $('#data_id').val(),
...
function takeLead(_data_id) {
    if(confirm('Are you sure to take/utilize lead?')) {
        $.ajax({
            type: 'post',
            url: URL + '/email-marketing-executive/data/take-lead-data',
            data: {
                data_id: _data_id,
                ca_agent_id: $('meta[name="ca-agent-id"]').attr('content')
            },
