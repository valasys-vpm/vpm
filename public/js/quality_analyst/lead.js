...
$(function (){
    ...
    LEAD_TABLE = $('#table-leads').DataTable({
        ...
        "serverSide": true,
        "ajax": {
            "url": URL + '/quality-analyst/lead/get-campaign-leads',
            data: {
                filters: function (){
                    let obj = {
                    };
        ...
        "columns": [
            {
                ...
                render: function (data, type, row) {
                    let html = '';
                    //html += '<a href="'+URL+'/quality-analyst/lead/edit/'+btoa(row.id)+'" class="btn btn-outline-secondary btn-rounded btn-sm" title="Edit lead details" style="padding: 2px 5px;"><i class="feather icon-edit mr-0" ></i></a>';
                    ...
                    if(parseInt(row.status)) {
                        ...
                        html += '<a href="javascript:void(0);" class="btn btn-outline-success btn-rounded btn-sm" onclick="approveLead(\''+btoa(row.id)+'\', \'' + row.qc_comment + '\')" title="Approve lead" style="padding: 2px 5px;"><i class="feather icon-check mr-0" ></i></a>';
                    }
                    return html;
                }
            },
    ...
    $('#form-reject-lead-submit').on('click', function (e) {
        ...
        $.ajax({
            type: 'post',
            url: URL + '/quality-analyst/lead/reject/' + $('#form-reject-lead').find('input[name="agent_lead_id"]').val(),
    ...
    $('#form-approve-lead-submit').on('click', function (e) {
        e.preventDefault();
        $.ajax({
            type: 'post',
            url: URL + '/quality-analyst/lead/approve/' + $('#form-approve-lead').find('input[name="agent_lead_id"]').val(),
            data: $('#form-approve-lead').serialize(),
            async : true,
            success: function (response) {
                if(response.status === true) {
                    trigger_pnofify('success', 'Successful', 'Agent\'s lead approved successfully');
                    $('#modal-approve-lead').modal('hide');
                    LEAD_TABLE.ajax.reload();
                } else {
                    trigger_pnofify('error', 'Something went wrong', response.message);
                }
...
function export_file(_id, _export_filter) {
    if(_id && confirm('Are you sure to export leads?')) {
        $.ajax({
            type: 'post',
            url: URL + '/quality-analyst/lead/export/' + _id,
            dataType: 'json',
            data: {
